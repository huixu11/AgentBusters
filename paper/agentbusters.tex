...
\subsection{Unified Scoring Methodology}

FAB++ implements a unified scoring system that normalizes all evaluator outputs to a 0-100 scale and computes a weighted overall score across three benchmark sections.

\subsubsection{Section-Based Scoring}

The overall score is computed as a weighted combination of three sections:
...
\begin{table}[H]
\centering
\caption{Benchmark Sections and Weights}
\label{tab:sections}
\begin{tabular}{llrl}
\toprule
\textbf{Section} & \textbf{Datasets} & \textbf{Weight} & \textbf{Focus} \\
\midrule
Knowledge Retrieval & BizFinBench, Public CSV & 30\% & Data extraction, financial facts \\
Analytical Reasoning & Synthetic Questions & 35\% & Logic puzzles, calculations \\
...
Different evaluators produce scores on different scales. We normalize all scores to 0-100:

\begin{table}[H]
\centering
\caption{Score Normalization Rules}
\label{tab:normalization}
\begin{tabular}{lll}
\toprule
\textbf{Evaluator} & \textbf{Input Range} & \textbf{Normalization} \\
\midrule
BizFinBench & 0.0--1.0 & $\text{score} \times 100$ \\
Public CSV & 0.0--1.0 & $\text{score} \times 100$ \\
Synthetic & 0.0--1.0 & $\text{score} \times 100$ \\
Options Alpha & 0--100 & No change \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Dynamic Weight Redistribution}

When some sections have no tasks (e.g., if synthetic questions are disabled), weights are redistributed proportionally among active sections:

\begin{equation}
w'_i = \frac{w_i}{\sum_{j \in \text{active}} w_j}
\end{equation}

This ensures the overall score remains meaningful regardless of which datasets are included in evaluation.
...
The following 22 questions comprise the Analytical Reasoning section. Questions are organized by topic with difficulty ratings (E=Easy, H=Hard, X=Expert).
